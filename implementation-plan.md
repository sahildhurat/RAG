...
**Architecture Pivot:** We upgraded the UI from a basic Streamlit app to a premium React frontend (built with Stitch) to improve aesthetics and user experience. To support this, we swapped the Streamlit backend for a REST API using FastAPI.

#### [NEW] [src/api.py](file:///d:/RAG/src/api.py)
- A lightweight FastAPI server.
- Exposes `POST /chat` which accepts a query, scrubs PII, invokes the LLM engine, and returns the response.
- Includes CORS middleware to allow the local React app to communicate with the backend.

#### [NEW] React Frontend (Stitch UI)
- A dark-themed, glassmorphic UI matching the problem statement's premium design requirements.
- Uses `fetch` to POST queries to `http://localhost:8000/chat`.

---

### Phase 5: Automated Testing

**Goal:** Ensure the RAG pipeline is robust, handles rate limits gracefully, respects factual constraints, and scrubs PII before deploying.

#### [NEW] [tests/test_api.py](file:///d:/RAG/tests/test_api.py)
- Integration tests using FastAPI's `TestClient` to verify the `/chat` and `/health` endpoints.
- Mocks the LLM response to test the API wrapper and CORS functionality without burning Groq API credits.

#### [NEW] [tests/test_logic.py](file:///d:/RAG/tests/test_logic.py)
- Unit tests for `src/utils.py` to ensure `scrub_pii` successfully redacts PAN cards and Aadhaar numbers.
- Unit tests for the `detect_fund()` router to ensure it maps shorthand names (like "axis silver") to the exact fund name.

#### [NEW] [tests/test_eval.py](file:///d:/RAG/tests/test_eval.py)
- E2E tests validating the LLM's adherence to rules (<= 3 sentences, contains citation, handles refusal).
- This will use a smaller subset of test queries to prevent hitting the 30 RPM rate limit on the 70B model.

### Phase 6: Scheduled Ingestion (GitHub Actions)

**Goal:** Automate the data ingestion pipeline so that the vector store is refreshed with the latest mutual fund data every day — without any manual intervention.

#### [NEW] [.github/workflows/scheduled-ingest.yml](file:///d:/RAG/.github/workflows/scheduled-ingest.yml)

A GitHub Actions workflow that:

1. **Runs on a cron schedule** — `cron: '30 18 * * *'` (18:30 UTC = midnight IST). This ensures the vector store is rebuilt with the freshest Groww page data before the next business day.
2. **Supports manual trigger** — `workflow_dispatch` is also enabled so developers can re-run ingestion on demand from the GitHub UI.

**Workflow Steps:**

| Step | Description |
| :--- | :--- |
| **Checkout** | `actions/checkout@v4` — clones the repo so `src/ingest.py` and all source files are available. |
| **Setup Python** | `actions/setup-python@v5` with `python-version: '3.11'` (matches local dev). |
| **Install dependencies** | `pip install -r requirements.txt` |
| **Run ingestion** | `python -m src.ingest` — executes the full scrape → clean → chunk → embed → persist pipeline from Phase 2. |
| **Commit & push updated DB** | Auto-commits any changes in `chroma_db/` and `ingested_data/` back to the repo so the deployed app always serves the latest embeddings. Uses `git diff --quiet` to skip the commit if nothing changed. |

**Secrets Required:**

- None for ingestion (no API key needed — embeddings are computed locally via HuggingFace).
- If the repo is private, the default `GITHUB_TOKEN` suffices for the push step.

> [!IMPORTANT]
> **Storage consideration:** ChromaDB's persisted files (`chroma_db/`) will be committed to the repo. For the current 10-fund dataset (~154 chunks) this is small (<50 MB). If the fund list grows significantly, consider switching to an external vector store (e.g., Pinecone, Weaviate) and storing only a connection string as a secret.

> [!NOTE]
> The workflow uses `git config` with a bot identity (`github-actions[bot]`) for the auto-commit so it doesn't pollute contributor history.

**Verification:**
- Push the workflow file and confirm it appears under the **Actions** tab in GitHub.
- Trigger a manual run via `workflow_dispatch` and verify the job completes successfully.
- Confirm that `chroma_db/` is updated in the repo after a successful run.
- Check the cron fires the next day by reviewing the workflow run history.
...
- **Unit & Integration:** Run `pytest tests/` to execute the automated test suite defined in Phase 5.
- **Coverage:** We will aim to ensure the PII scrubber and API endpoints have 100% test coverage.
